...
public class ObstacleMap : TileMapController
{
    ...
    private void DoStart()
    {
        ...
        {
            // 위에서 아래로 서치한다.
            searchTerrains = mapController.GetTerrain_Colum(searchIdx, true);
        ...
        {
            // 아래에서 위로 서치한다
            searchTerrains = mapController.GetTerrain_Colum(searchIdx);
            foreach(var searchTerrain in searchTerrains)
            {
                if(searchTerrain.IsPassable)
                {
        ...
        GameObject changeTilePrefab = 
            ResManager.Instance.obstaclePrefabs[RDefine.OBSTACLE_PREF_PLAIN_CASTLE];
