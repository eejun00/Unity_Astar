...
public class TerrainMap : TileMapController
{
    ...
    private void Start()
    {
        ...
        GameObject changeTilePrefabs = ResManager.Instance.terrainPrefabs
        ...
        for (int i = 0; i < allTileObjs.Count; i++)
        {
            ...
            tempChangeTile = Instantiate(
                changeTilePrefabs, tileMap.transform);
        ...
        // { 기존에 존재하는 타일의 순서를 조정하고, 컨ㅌ롤러를 캐싱하는 로직
        ...
        foreach(GameObject tile_ in allTileObjs)
        {
            ...
            tempTerrain.SetupTerrain(mapController, terrainType, loofCount);
