...
public class PathFinder : GSingleton<PathFinder>
{
    ...
    private IEnumerator DelayFindPath_Astar(float delay_)
    {
        ...
        while(isFoundDestination == false && isNowayToGo == false )
        {
            ...
            foreach(var terrainNode in aStarOpenPath)
            {
                ...
                }   // else: 가장 작은 코스트의 노드가 캐싱 되어있는 경우
            ...
            else
            {
                ...
                List<int> nextSearchIdx1Ds = 
                    mapBoard.GetTileIdx2D_Around4ways(minCostNode.Terrain.TileIdx2D);
                // 찾아온 노드 중에서 이동 가능한 노드는 Open 리스트에 추가한다.
                ...
                }   // loop: 이동가능한 노드를 Open 리스트에 추가하는 루프
                // } 도착하지 않았다면 현재 타일을 기준으로 4방향 노드를 찾아온다.

                // 탐색이 끝난 노드는 Close list에 추가하고, Open 리스트에서 제거한다.
                // 이 때, Open 리스트가 비어있다면 더이상 탐색할 수 있는 길이 존재하지 않는것이다.
                aStarClosePath.Add(minCostNode);
                aStarOpenPath.Remove(minCostNode);
                if(aStarOpenPath.IsValid() == false)
                {
                    GFunc.LogWarning("[Warning] 길이 존재하지 않습니다.");
                    isNowayToGo = true;
    ...
    //! Target 지형 정봐와 Destination 지형 정보로 Distance와 Heuristic을 설정하는 함수
